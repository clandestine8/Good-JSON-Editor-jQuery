	...
	$.fn.goodJSON.defaultSettings = {
		...
		open_change_field : function (settings, target) {
			...
			$.each(_acceptedType, function(typeName) {
				...
				if (target.is(settings._base) && "[object Object]" !== typeName && "[object Array]" !== typeName)
					return;
		...
		on: {
			...
			dragenter: {
				...
				'json-indent' : function (settings, evnt) {
					...
					if (!settings._isDragging || jQuery.inArray( settings._currentDraggedSource[0], evnt.originalEvent.path ) >= 0 || settings._currentDraggedSource.is(this))
						return false;
