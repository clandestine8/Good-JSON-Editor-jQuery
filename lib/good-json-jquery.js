...
(function( $ ) {
		...
		, _loop = {
			objectify : function (parent, res, settings) {
				$.each(parent.childrenWithData('type'), function(i, value) {
					...
					res[$(value).data('field')] = methods.objectify.apply( value, [settings] );
		...
		, _history = {
			_add: function (settings) {
				...
				if (settings._history.list.length > settings._history.cur) {
					...
					for (var x = settings._history.cur; x < settings._history.max; x++)
				...
				if (settings._history.cur >= settings._history.max) {
					settings._history.cur --;
	...
	$.fn.goodJSON.defaultSettings = {
		...
		acceptedType : {
			"[object String]" : {
				edit: function (settings, originalData) {
					var r =  $('<textarea rows="' + (originalData.split(/<br>/).length) + '">'+originalData.replace(/<br>/g, "\n")+'</textarea>');
			...
			"[object Number]" : {
				edit: function (settings, originalData) {
					return $('<input type="number" step="any" value="'+originalData+'" />');
				},
			...
			"[object Boolean]" : {
				edit: function (settings, originalData) {
					return $('<span>'+(originalData === 'true' ? 'false' : 'true')+'</span>').click(function (_) {
						$(this).parent().click();
					});
				},
				value: function (settings) {
					return settings.inputActive.text();//.children("#t").is(":checked") ? 'true' : 'false';
				}
			}
		},
		icons : {
		...
		classes : {
			...
			'.json-type' : "margin-right:5px; margin-left:5px; font-family: 'Custom Icons';text-align: center;font-size: 12px;text-decoration: underline;",
		...
		open_new_edit: function (settings) {
			...
			if (this.hasClass(settings.elements.field))
				settings.inputActive = $('<input type="text" value="' + this.text() + '" />');
		...
		change_type: function (settings, target, newType, oldType) {
			...
			var oldValue = methods.objectify.apply(target, [settings]),
				newValue = _acceptedType[newType].change(settings, oldValue, oldType),
				newObject = _populateOpts(_parseType(settings, newValue), settings, target.attr('data-field'));
		...
		on: {
			...
			dragenter: {
				...
				'json-indent' : function (settings, evnt) {
					...
					if (!settings._isDragging || jQuery.inArray( settings._currentDraggedSource[0], evnt.originalEvent.path ) >= 0 || this[0] === settings._currentDraggedSource[0])
						return false;
